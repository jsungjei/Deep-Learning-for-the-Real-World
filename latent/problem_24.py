...
def visualize_receptive_fields(W, filename):
    W_cols = W.shape[1]
    fig= plt.figure()
    fig.set_size_inches(25, 25)
    dimension_size = numpy.ceil(numpy.sqrt(W_cols))
    for i in xrange(W_cols):
        data = W[:,i]
        data = data / data.max()
        img = data.reshape(28, 28).astype('float32')
        subplot = fig.add_subplot(dimension_size, dimension_size, i+1)
...
if __name__ == '__main__':
    ...
    SqAE = SparseAutoEncoder(num_input=28 * 28, num_hidden=m_num_hidden, sparsity_lambda=m_sparsity_lambda)
    data_loader = load_data()
    [(train_set_x, train_set_y), (valid_set_x, valid_set_y),
    ...
    autoencoderfilter_figure_name = str(m_sparsity_lambda) + "_autoencoderfilter.png"
    visualize_receptive_fields(SqAE.W.get_value(borrow=True), autoencoderfilter_figure_name)
    visualize_receptive_fields(SqAE.W.get_value(borrow=True), autoencoderfilter_figure_name)
